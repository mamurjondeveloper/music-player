...
export default function ProfilePage() {
  ...
  const handleGenerateInvite = async () => {
    ...
    try {
      ...
      setInviteCodes((prev) => [{ ...res.data, usedBy: null, usedAt: null, createdAt: new Date().toISOString() }, ...prev]);
  ...
  return (
    <div className="space-y-8 pb-12 max-w-2xl">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Manage your account, security, and invites</p>
      </div>
      ...
      <div className="glass-panel p-6 rounded-3xl space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <KeyRound className="h-4 w-4 text-primary" /> Invite Friends
          </h2>
          <Button onClick={handleGenerateInvite} isLoading={isGeneratingInvite} className="text-xs px-3 py-2">
            Generate Code
          </Button>
        </div>
        <p className="text-xs text-gray-400">
          Each code is single-use. Share one with a friend so they can register on the Register page.
        </p>

        {isLoadingInvites ? (
          <div className="text-center text-sm text-gray-500 py-4">Loading...</div>
        ) : inviteCodes.length === 0 ? (
          <div className="text-center text-sm text-gray-500 py-4">No invite codes yet — generate one above.</div>
        ) : (
          <div className="space-y-2 max-h-72 overflow-y-auto no-scrollbar">
            {inviteCodes.map((invite) => (
              ...
              >
                <span className="font-mono text-sm font-bold tracking-widest text-white select-all">
                  {invite.code}
                </span>
                {invite.usedBy ? (
                  <span className="flex items-center gap-1.5 text-xs text-gray-500 font-semibold shrink-0">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="flex items-center gap-1.5 text-xs text-primary font-semibold">
                      <Clock className="h-3.5 w-3.5" /> Unused
                    </span>
                    <button
